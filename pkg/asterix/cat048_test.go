...
func TestCAT048DecodeTyped(t *testing.T) {
	// Test the new DecodeTyped method
	tests := []struct {
		name    string
		hexData string
		wantSIC uint8
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Decode raw message first
			rawMsg := &RawAsterixMessage{
				Category: tt.wantCat,
				Length:   uint16(len(data)),
				Payload:  data[3:], // Skip category and length bytes
			}
			...
			if err != nil {
				t.Fatalf("Failed to decode typed message: %v", err)
			}

			// Verify basic fields
			if typedMsg.Category != tt.wantCat {
				t.Errorf("Category = %d, want %d", typedMsg.Category, tt.wantCat)
			}

			if typedMsg.SAC != tt.wantSAC {
				t.Errorf("SAC = %d, want %d", typedMsg.SAC, tt.wantSAC)
			}

			if typedMsg.SIC != tt.wantSIC {
				t.Errorf("SIC = %d, want %d", typedMsg.SIC, tt.wantSIC)
			}
			...
			if typedMsg.DataSourceIdentifier == nil {
				...
				if typedMsg.DataSourceIdentifier.SAC != tt.wantSAC {
					t.Errorf("DataSourceIdentifier.SAC = %d, want %d", typedMsg.DataSourceIdentifier.SAC, tt.wantSAC)
				}
				if typedMsg.DataSourceIdentifier.SIC != tt.wantSIC {
					t.Errorf("DataSourceIdentifier.SIC = %d, want %d", typedMsg.DataSourceIdentifier.SIC, tt.wantSIC)
				}
			}
			...
			// Log which typed fields were populated
			var populatedFields []string
			if typedMsg.DataSourceIdentifier != nil {
				populatedFields = append(populatedFields, "DataSourceIdentifier")
			}
			if typedMsg.TimeOfDay != nil {
				populatedFields = append(populatedFields, "TimeOfDay")
			}
			if typedMsg.TargetReportDescriptor != nil {
				populatedFields = append(populatedFields, "TargetReportDescriptor")
			}
			if typedMsg.MeasuredPosition != nil {
				populatedFields = append(populatedFields, "MeasuredPosition")
			}
			if typedMsg.AircraftIdentification != nil {
				populatedFields = append(populatedFields, "AircraftIdentification")
			}
			if typedMsg.ModeACode != nil {
				populatedFields = append(populatedFields, "ModeACode")
			}
			if typedMsg.FlightLevel != nil {
				populatedFields = append(populatedFields, "FlightLevel")
			}
			if typedMsg.RadarPlotCharacteristics != nil {
				populatedFields = append(populatedFields, "RadarPlotCharacteristics")
			}
			if typedMsg.CalculatedPosition != nil {
				populatedFields = append(populatedFields, "CalculatedPosition")
			}
			if typedMsg.CalculatedPositionCartesian != nil {
				populatedFields = append(populatedFields, "CalculatedPositionCartesian")
			}

			t.Logf("Populated typed fields: %v", populatedFields)
		})
	}
}
